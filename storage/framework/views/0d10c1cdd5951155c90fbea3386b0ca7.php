...
<?php $__env->startSection('title', 'Market Scanner'); ?>
<?php $__env->startSection('page-title', 'Market Scanner'); ?>
    ...
    <div class="panel-body no-padding" style="overflow-x:auto;max-height:calc(100vh - 220px);overflow-y:auto">
        <table class="data-table scanner-stripe" id="scanner-table">
            ...
            <tbody id="scanner-body">
                <?php $__currentLoopData = $pairs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr data-symbol="<?php echo e($p['symbol']); ?>" data-pair="<?php echo e($p['pair']); ?>" onclick="window.location='/trading?symbol=<?php echo e($p['symbol']); ?>'" class="hover-glow">
                    <td style="color:var(--text-muted)"><?php echo e($i + 1); ?></td>
                    <td style="font-weight:600"><?php echo e($p['pair']); ?></td>
                    <td class="align-right scanner-price" style="font-family:var(--font-mono)">$<?php echo e(number_format($p['price'], $p['price'] < 1 ? 6 : 2)); ?></td>
                    <td class="align-right scanner-change <?php echo e($p['change'] >= 0 ? 'positive' : 'negative'); ?>" style="font-family:var(--font-mono)">
                        <?php echo e($p['change'] >= 0 ? '+' : ''); ?><?php echo e(number_format($p['change'], 2)); ?>%
                    </td>
                    <td class="align-right" style="font-family:var(--font-mono);color:var(--text-muted)">$<?php echo e(number_format($p['high'], $p['high'] < 1 ? 6 : 2)); ?></td>
                    <td class="align-right" style="font-family:var(--font-mono);color:var(--text-muted)">$<?php echo e(number_format($p['low'], $p['low'] < 1 ? 6 : 2)); ?></td>
                    <td class="align-right" style="font-family:var(--font-mono)"><?php echo e($p['quoteVolume'] > 1e9 ? number_format($p['quoteVolume'] / 1e9, 2) . 'B' : number_format($p['quoteVolume'] / 1e6, 1) . 'M'); ?></td>
                    <td class="align-right" style="font-family:var(--font-mono)"><?php echo e(number_format($p['trades'])); ?></td>
...
<div class="kpi-grid">
    <div class="kpi-card">
        ...
        <div class="kpi-value text-mono"><?php echo e(count($pairs)); ?></div>
    </div>
    <div class="kpi-card">
        <div class="kpi-label">Gainers</div>
        <div class="kpi-value text-mono text-success"><?php echo e(collect($pairs)->filter(fn($p) => $p['change'] > 0)->count()); ?></div>
    </div>
    <div class="kpi-card">
        <div class="kpi-label">Losers</div>
        <div class="kpi-value text-mono text-danger"><?php echo e(collect($pairs)->filter(fn($p) => $p['change'] < 0)->count()); ?></div>
    </div>
    <div class="kpi-card">
        <div class="kpi-label">Avg Change</div>
        <?php $avgChange = count($pairs) > 0 ? collect($pairs)->avg('change') : 0; ?>
        <div class="kpi-value text-mono <?php echo e($avgChange >= 0 ? 'text-success' : 'text-danger'); ?>"><?php echo e(number_format($avgChange, 2)); ?>%</div>
    </div>
</div>
...
let sseSource = null;
function initSSE() {
    sseSource = new EventSource('<?php echo e(url('/api/market/stream')); ?>?page=scanner');
...
function refreshScannerTable() {
    ...
    fetch('<?php echo e(url('/api/market/top-pairs')); ?>')
        .then(r => r.json())
        .then(pairs => {
...
function loadSignalsForTopPairs() {
    ...
    symbols.forEach(symbol => {
        fetch(`<?php echo e(url('/api/market/prediction')); ?>?symbol=${symbol}&interval=15m`)
            .then(r => r.json())
            .then(data => {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Project\market_pro\resources\views/scanner/index.blade.php ENDPATH**/ ?>