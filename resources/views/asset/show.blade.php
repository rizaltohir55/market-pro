...
@section('title', 'Asset Detail — ' . $symbol)
@section('page-title', 'Asset Analysis: ' . ($profile['name'] ?? $symbol))
...
<div style="display:flex;align-items:center;gap:var(--space-4);margin-bottom:var(--space-4)">
    ...
    <span class="kpi-delta {{ (float)$quote['change_pct'] >= 0 ? 'positive' : 'negative' }}" id="asset-change">
...
<div class="grid-trading-immersive fade-in-up" style="--delay: 0.1s; display: flex; flex-direction: row; gap: var(--space-4); height: 60vh; min-height: 500px; margin-bottom: var(--space-4);">
    ...
    <div class="side-dock fade-in-up" style="flex: 1; display: flex; flex-direction: column; gap: var(--space-4);">
        <div class="panel acrylic" style="flex: 1; overflow-y: auto;">
            ...
            <div class="panel-body no-padding">
                <div style="padding: var(--space-4); display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-4);">
                    @if($profile && isset($profile['fundamentals']))
                        ...
                        <div class="stat-item">
                            ...
                            <div class="stat-value">${{ number_format((float)($f['eps_ttm'] ?? 0), 2) }}</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-label">Div Yield</div>
                            <div class="stat-value">{{ number_format(((float)($f['dividend_yield']??0))*100, 2) }}%</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-label">Price to Book</div>
                            <div class="stat-value">{{ number_format((float)($f['price_to_book']??0), 2) }}</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-label">ROE</div>
                            <div class="stat-value">{{ number_format(((float)($f['roe']??0))*100, 2) }}%</div>
                        </div>
                    @else
                        {{-- Fallback for Crypto / Forex / Commodities --}}
                        <div class="stat-item">
        ...
        @if($type === 'stock')
        {{-- Analyst Recommendations (Stocks only) --}}
