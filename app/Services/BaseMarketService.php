...
abstract class BaseMarketService
{
    ...
    protected function getHttpOptions(int $timeout = 5): array
    {
        ...
        // Robust SSL verification handling for Windows/Local environments
        $verify = true;
        
        if ($cacert === false || $cacert === 'false' || $cacert === 0 || $cacert === '0') {
            $verify = false;
        } elseif (is_string($cacert) && !empty($cacert) && file_exists($cacert)) {
            $verify = $cacert;
        }
